...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(
        value = "select employee from Employee employee left join fetch employee.department left join fetch employee.location",
        countQuery = "select count(employee) from Employee employee"
    )
    Page<Employee> findAllWithToOneRelationships(Pageable pageable);

    @Query("select employee from Employee employee left join fetch employee.department left join fetch employee.location")
    List<Employee> findAllWithToOneRelationships();

    @Query("select employee from Employee employee left join fetch employee.department left join fetch employee.location where employee.id =:id")
